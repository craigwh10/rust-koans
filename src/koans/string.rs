...
fn using_chars() {
    ...
    assert!(string.chars().nth(1) == string.chars().nth(10));
}
...
fn slicing_the_string() {
    let string = "Boom";
    println!("string {}", &string[0..3]); // 0..2 returns 0,1 - 0..3 returns 0,1,2
                                          // which is weird but is behaviour.
    assert!(&string[0..3] == "Boo");
}
