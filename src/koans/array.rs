...
fn array_index() {
    ...
    // Need to assign as a i32 to compare
    // as you can only compare the same types.
...
#[allow(const_err, unconditional_panic)] // had to add unconditional panic allow because its deny by default
fn out_of_index() {
...
fn complex_array_filter_map() {
    ...
    let mut iterator = arr.iter() // f64 is float.
